...
class Job():
	def __init__(self, _id, size_bits, serv_time, typ='local'):
		self._id = _id
	...
	def __repr__(self):
		return "Job[id= {}, size_bits= {}, serv_time= {}, typ= {}]".format(self._id, self.size_bits, self.serv_time, self.typ)
...
class MasterQ(FCFS):
	...
	def __repr__(self):
		return "MasterQ[id= {}]".format(self._id)
...
class OverloadedQ(FCFS):
	def __init__(self, _id, env, out, speed=1):
		super().__init__(_id, env, out, speed)

	def __repr__(self):
		return "OverloadedQ[_id= {}]".format(self._id)
